...
- `docs_reviewed[]` is a `jsonb` column on `cases` (headline-only, small, loaded with the case and injected client-side alongside `intake` — comparable shallow sensitivity; the server-side-only layer is `context_narrative`, not these). The headline is generated by a deliberately shallow `HEADLINE_EXTRACT_PROMPT` (floor **and** ceiling — explicitly **not** the rich `IEP_EXTRACT_PROMPT`). The raw upload text is never persisted: it lives only in the in-tab `uploadedDocs` state for the active session and dies with the tab.
- `generated_docs[]` are stored as **PDF blobs** (byte-identical to what the parent downloads, not a re-render) in a **private** Supabase Storage bucket `generated-docs`, under a per-user folder `<uid>/…`. Access is the storage twin of the table RLS — four `storage.objects` policies scoped to `(storage.foldername(name))[1] = auth.uid()::text`, authenticated-only, anon denied by RLS default-deny (see `supabase/storage.sql`). The account page lists the bucket folder directly; there is no metadata table (the `source_session` linkage from Section 3 is dropped for v1, recoverable later). The cross-user storage gate (B-vs-A on all four ops + an anon attempt) was run and passed before any real file was written.
- **Step 5 copy-up hooks:** the write paths are factored as standalone, callable units — `extractHeadlineForDoc` / `appendDocReviewed` for the ledger, and `pdfBlobFromText` / `saveGeneratedDoc` for the bucket — so the deferred anonymous→account upgrade (Flow 2) can replay them over the in-tab `uploadedDocs` and generated `outputs`/`draft` at save time, with no rewrite. Anonymous use still writes **zero** server rows and zero bucket objects; every Step 3 write path is gated on an authenticated user.
...
- **delete-my-data UI** — v1.1, planned. **Storage-cascade obligation (write it down):** deleting an auth user cascades the `cases`/`sessions` **rows** (FK `on delete cascade`), but storage objects live in the storage subsystem and are **NOT** reached by that cascade. The v1.1 delete path must therefore explicitly remove the user's bucket folder. Because every generated-doc object is uid-prefixed (`<uid>/…`), that deletion is a clean one-liner — `storage.from('generated-docs').remove(<objects under "<uid>/">)` — which is exactly why the uid-prefixed path was chosen now. (Also noted in `supabase/storage.sql`.)
